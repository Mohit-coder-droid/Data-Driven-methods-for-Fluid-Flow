...
G(:,a)    = SPIDER_integrate( U, [1], grid, corners, size_vec, pol ) + ...
            SPIDER_integrate( W, [2], grid, corners, size_vec, pol );
scales(a) = 1;
a = a+1;
...
%labels{a} =  "T^3";
%G(:,a)    = SPIDER_integrate( T.*T.*T, [], grid, corners, size_vec, pol );
%scales(a) = 1;
%a = a+1;

labels{a} =  "U_i.\nabla T";
G(:,a)    = SPIDER_integrate( U.*dxT + W.*dzT, [], grid, corners, size_vec, pol );
scales(a) = 1;
a = a+1;

labels{a} =  "\nabla^2 T";
G(:,a)    = SPIDER_integrate( T, [1,1], grid, corners, size_vec, pol ) + ...
	    SPIDER_integrate( T, [2,2], grid, corners, size_vec, pol );
scales(a) = 1;
a = a+1;

labels{a} =  "T T_t";
G(:,a)    = SPIDER_integrate( T.*dtT, [], grid, corners, size_vec, pol );
scales(a) = 1;
a = a+1;
...
labels{a} =  "T (\nabla_i.u_i)";
G(:,a)    = SPIDER_integrate( T.*(dxU + dzW), [], grid, corners, size_vec, pol );
scales(a) = 1;
a = a+1;

labels{a} =  "U^2.T";
G(:,a)    = SPIDER_integrate( (U.*U + W.*W).*T, [], grid, corners, size_vec, pol );
scales(a) = 1;
a = a+1;
