...
% Now convert to lower boundary data only
Tbc = squeeze(T(:,1,:));
dxTbc = squeeze(dxT(:,1,:));
dzTbc = squeeze(dzT(:,1,:));

% Now convert to upper boundary data only
%szdim2 = size(T,2);
%Tbc = squeeze(T(:,szdim2,:));
%dxTbc = squeeze(dxT(:,szdim2,:));
%dzTbc = squeeze(dzT(:,szdim2,:));
...
size_of_data = size(Tbc, 1:dimension);
...
labels{a} = "T";
G(:,a)    = SPIDER_integrate( Tbc, [], grid, corners, size_vec, pol );
scales(a) = 1;
a = a+1;

labels{a} = "1";
G(:,a)    = SPIDER_integrate( 0*Tbc + 1, [], grid, corners, size_vec, pol );
scales(a) = 1;
a = a+1;

labels{a} = "T_x";
G(:,a)    = SPIDER_integrate( dxTbc, [], grid, corners, size_vec, pol );
scales(a) = 1;
a = a+1;

labels{a} = "T_z";
G(:,a)    = SPIDER_integrate( dzTbc, [], grid, corners, size_vec, pol );
scales(a) = 1;
a = a+1;
